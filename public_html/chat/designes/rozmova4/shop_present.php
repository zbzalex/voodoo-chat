...
<form method="post" name="myForm" action="shop_submit.php">
    ...
    <table align="center">
        <tr>
            <td><?= $w_roz_user ?> :</td>
            <td><input class="input" type="Text" name="look_for" value="">&nbsp; <input class="input_button"
        ...
        <?php if (count($u_ids)) { ?>
                ...
                <td>
                    <select name="present_for">
                        ...
                        for ($i = 0; $i < count($u_ids); $i++) {
                            ?>
                            <option value="<?= $u_ids[$i]; ?>"
                                    <?php if (strcasecmp($u_names[$i], $look_for) == 0) { ?>SELECTED<?php } ?>><?= $u_names[$i] ?></option>
                            <?php
                        }
                        ?>
                    </select>
            ...
            <tr>
                <td colspan="2"><input class="input_button" type="Button" value="<?= $w_shop_present ?>"
                                       onclick="buy();"></td>
            </tr>
        <?php } ?>
    </table>
