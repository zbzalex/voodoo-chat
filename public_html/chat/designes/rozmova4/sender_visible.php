    ...
    <script>
        function run_cmd(cmdType) {
            ...
            if (cmdType == "do_damn" || cmdType == "do_undamn" || cmdType == "do_reward") {
                if (victim.length == 0 || msg.length == 0) {
                    alert('<?php echo $w_roz_need_cause; ?>');
                    return;
                }
                IsCorrect = true;
            }
            ...
            if (cmdType == "do_announce") {
                if (msg.length == 0) {
                    alert('<?php echo $w_roz_add_announce; ?>');
                    return;
                }
                IsCorrect = true;
            }
            if (cmdType == "do_alert") {
                if (victim.length == 0 || msg.length == 0) {
                    alert('<?php echo $w_roz_add_alert; ?>');
                    return;
                }
                IsCorrect = true;
            }
            ...
            <?php } ?>) {
                if (victim.length == 0) {
                    alert('<?php echo $w_roz_add_clear; ?>');
                    return;
                }
                IsCorrect = true;
            }
            ...
            if (cmdType == "do_silence" || cmdType == "do_chaos") {
                if (victim.length == 0 || msg.length == 0) {
                    alert('<?php echo $w_roz_add_silence; ?>');
                    return;
                }
                IsCorrect = true;
            }
...
?>
    ...
    <body <?php if ($opcode == "popup") { ?>onLoad="document.forms[0].whisper.value=parent.WhisperWith;" <?php } ?>
          bgcolor=#BCD560 leftmargin="0" topmargin="0" marginwidth="0" marginheight="0"
          background="./img/down_buttons_menu.jpg">
    <SCRIPT>
    ...
    <form action="<?php echo $chat_url ?>sender.php" method="post" name="cmdBar" target="voc_sender_hidden"
          onSubmit="msg_submit(); return false;">
                    ...
                    <table border="0" cellspacing="0" cellpadding="1" width=100%>
                        ...
                        <table width=100% cellspacing="0" cellpadding="0">
                            <tr>
                                ...
                                <td width="<?php if ($current_user->use_old_paste == 1) { ?>105<?php } else echo "140"; ?>"
                                    align=CENTER valign=middle><input name="whisper" type="text" size=8
                                                                      style="width:100px;"
                                                                      class=flat <?php if ($opcode == "popup") echo "readonly"; ?>>&nbsp;
                                ...
                                <td valig="middle" align="left"><a href="#" tabindex="200"><img tabindex="200"
                                                                                                src="<?= $current_design ?>grunge/er.gif"
                                                                                                wudth="16" height="16"
                                                                                                onClick=msgdecode()
                                                                                                border="0"></a></td>
                                <td><input type="button" class=input_button value="<?= $w_whisper ?>"
                        ...
                        <table cellspacing="0" cellpadding="0">
                            <tr>
                                <td>&nbsp;<input type="button" class=input_button value="<?= $w_roz_profile ?>"
                                                 onClick="whoIs();">&nbsp;
                                </td>
                                <td><input type=checkbox
                                           name="clr_to" <?php if ($current_user->use_old_paste == 0 and $allow_multiply) { ?> checked <?php } ?>>
                                    <small><?php echo $w_clear_nick_after; ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</small></td>
                                <td valign="middle">
                                    ...
                                    if (function_exists("translit_" . strtolower(trim($cu_array[USER_LANG]))) > 0) {
                                        ?>
                                        <input name="translit" type="checkbox" value=1>
                                ...
                                <td>&nbsp;&nbsp;</td>
                                <?php if (strlen(trim($current_user->style_start)) > 0 and strlen(trim($current_user->style_end)) > 0) {
                                    ?>
                                    <td><input name="custom_style" type="checkbox"
                                               value=1><small><?php echo $w_roz_style; ?></small></td>
                                    <?php
                                }
                                ?>
                                <td></td>
                        ...
                        <table width=100%>
                                    ...
                                    <td valign=middle>
                                        ...
                                        <?php if ($current_user->custom_class & CST_PRIEST) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_roz_damn_cmd; ?>" onClick=run_cmd('do_damn')>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_roz_undamn_cmd; ?>"
                                                   onClick=run_cmd('do_undamn')>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_roz_reward_cmd; ?>"
                                                   onClick=run_cmd('do_reward')>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_admin_alert; ?>" onClick=run_cmd('do_alert')>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_roz_silence; ?>" onClick=run_cmd('do_silence')>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
                            <?php if ($cu_array[USER_CLASS] > 0) { ?>
                                <tr>
                                    <td colspan=5 align=right>
                                        ...
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_roz_announce; ?>"
                                                   onClick=run_cmd('do_announce')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_admin_alert; ?>" onClick=run_cmd('do_alert')>
                                        ...
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_roz_clear_pub; ?>" onClick=run_cmd('do_clear')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN) { ?>
                                            <input type="button" class=input_button value="<?php echo $w_roz_jail; ?>"
                                                   onClick=run_cmd('do_jail')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_roz_silence; ?>" onClick=run_cmd('do_silence')>
                                            <input type="button" class=input_button value="<?php echo $w_adm_chaos; ?>"
                                                   onClick=run_cmd('do_chaos')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_adm_level[ADM_BAN]; ?>"
                                                   onClick=run_cmd('do_ban')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_IP_BAN) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_adm_level[ADM_IP_BAN]; ?>"
                                                   onClick=run_cmd('do_ip_ban')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN_BY_SUBNET) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_adm_level[ADM_BAN_BY_SUBNET]; ?>"
                                                   onClick=run_cmd('do_subnet_ban')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN_BY_BROWSERHASH) { ?>
                                            <input type="button" class=input_button
                                                   value="<?php echo $w_adm_level[ADM_BAN_BY_BROWSERHASH]; ?>"
                                                   onClick=run_cmd('do_hash_ban')>
                                        <?php } ?>
                                        <?php if ($cu_array[USER_CLASS] & ADM_BAN_BY_BROWSERHASH) { ?>
                                            <input type="button" class=input_button value="<?php echo Quake; ?>"
                                                   onClick=run_cmd('do_ring')>
                                        <?php } ?>
                                    </td>
                                </tr>
                                <?php if ($cu_array[USER_CLASS] & ADM_VIEW_PRIVATE) { ?>
                                    <tr>
                                        <td colspan=5 align=RIGHT>
                                            <input type="button" class=input_button value="redirect"
                                                   onClick=run_cmd('do_redirect')>
                                            <input type="button" class=input_button value="comatoze"
                                                   onClick=run_cmd('do_multiplywindows')>
                                            <input type="button" class=input_button value="reboot"
                                                   onClick=run_cmd('do_shutdown')>
                                        </td>
                                    </tr>
                                <?php }
                            } ?>
