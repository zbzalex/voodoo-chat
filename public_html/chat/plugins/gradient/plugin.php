...
function Gradient_Do($UserNick, &$Msg)
{
    ...
    if ($custom_style == 1 and $is_regist_complete) {
        ...
        if ($current_user->plugin_info["gradient_color_start"] != $current_user->plugin_info["gradient_color_end"]) {
            ...
            if ($current_user->plugin_info["gradient_color_start"] == "") {
                $current_user->plugin_info["gradient_color_start"] = $default_color;
            }
            if (($current_user->plugin_info["gradient_color_start"] < 0) or ($current_user->plugin_info["gradient_color_start"] >= count($registered_colors))) {
                $current_user->plugin_info["gradient_color_start"] = $default_color;
            }

            if ($current_user->plugin_info["gradient_color_end"] == "") {
                $current_user->plugin_info["gradient_color_end"] = $default_color;
            }
            if (($current_user->plugin_info["gradient_color_end"] < 0) or ($current_user->plugin_info["gradient_color_end"] >= count($registered_colors))) {
                $current_user->plugin_info["gradient_color_start"] = $default_color;
            }
